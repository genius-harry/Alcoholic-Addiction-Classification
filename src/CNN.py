...
Original file is located at
    https://colab.research.google.com/drive/1xYMlQvNEwn1m-fyRPFCqGyToXbk7Ra15
"""
...
# print(S_images_train_classifier.shape)
# print(Flat_S_images_train_classifier.shape)
...
class CNNModel(nn.Module):
    ...
    def forward(self, x):
        ...
        # Flatten the output for the feedforward layers
        x = x.view(x.size(0), -1)
        return x

class ModifiedFeedforwardNeuralNetModel(nn.Module):
    def __init__(self, hidden_size):
        super(ModifiedFeedforwardNeuralNetModel, self).__init__()
        self.cnn = CNNModel()
        # Calculate the size of the flattened CNN output and use as input_size for the linear layer
        # Example: self.fc1 = nn.Linear(32 * 8 * 8, hidden_size)
        self.fc1 = nn.Linear(2048, hidden_size)  # Adjust 2048 based on your CNN's output
        self.relu = nn.ReLU()
...
print()
#Train ModifiedFeedforwardNeuralNetModel

model = ModifiedFeedforwardNeuralNetModel(3072)
...
model.eval()
y_pred2 = model(S_images_test_classifier)
...
model.train()
Model_images_train_classifier = FFtrainer(S_images_train_classifier,S_y_train_classifier,model,FFoptimizer2)


# Evaluation

model.eval()
y_pred = model(S_images_test_classifier)
